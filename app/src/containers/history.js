            ...
            <div className="w-full px-4 mx-auto lg:px-4">
                <div className="relative bg-white shadow-md dark:bg-gray-800 sm:rounded-lg">
                    <div className="flex-row items-center justify-between p-4 space-y-3 sm:flex sm:space-y-0 sm:space-x-4">
                        <DatePickerButton onChangeAct={handleChangeDate} title=" " value={from} campo="date" conFecha={true} />
                        <label className='text-white'>To</label>
                        <DatePickerButton onChangeAct={handleChangeDate} title=" " value={to} campo="date" conFecha={true} />

                        <div className="flex">
                            <div className="inline-flex rounded-md shadow-sm w-100 flex-grow" role="group">
                                <button type="button" onClick={() => setIdCategory(0)} className={`flex-grow px-4 py-2 text-sm font-medium text-gray-900  border border-gray-900 rounded-l-lg hover:bg-gray-900 hover:text-white focus:z-10 focus:ring-2 focus:ring-gray-500 focus:bg-gray-900 focus:text-white dark:border-white dark:text-white dark:hover:text-white dark:hover:bg-lime-800 dark:focus:bg-lime-800 ${operation == 0 ? " bg-lime-600" : "bg-transparent"} `}>
                                    All
                                </button>
